...
E.DESCRIPCION,
F.CODIGO_DIRECCION,
/*decode(D.tipo_de_persona, 'N',D.primer_apellido || 
decode(D.segundo_apellido, null, '', ' ' ||D.segundo_apellido) || 
decode(D.apellido_de_casada,null, '', ' DE '||D.apellido_de_casada)|| ', ' ||
D.nombres,D.nombre_comercial)                                                  CLIENTE*/
...
AND A.CODIGO_TIPO_TRANSACCION = 18
...
--FUNCIONES:
-- F_DESCRIPCION
function CF_DESCRIPCIONFormula return Char is
...
-- F_VALOR_MONTO:
function CF_VALORFormula return Char is
...
-- F_saldoanterior
function CF_saldoanteriorFormula return Char is
Ln_Saldo_Total_ant   number(14,2);
Lv_SaldoAnterior     varchar2(30);
begin
  Ln_Saldo_Total_ant := nvl(:cf_saldototal,0) + nvl(:cs_suma_monto,0); --LLAMA A OTRA FUNCION
  lv_SaldoAnterior :=  :abreviatura||' '||ltrim(to_char(nvl(Ln_Saldo_Total_ant,0),'999,999,999,999,990.00')); 
  RETURN(lv_SaldoAnterior);
end;

-- F_nuevo_saldo
function CF_nuevo_saldoFormula return Char is
Lv_saldo_total   varchar2(30);
begin
  lv_saldo_total :=  :abreviatura||' '||ltrim(to_char(nvl(:cf_saldototal,0),'999,999,999,999,990.00'));  -- LLAMA A OTRA FUNCION
  RETURN(Lv_Saldo_Total);
end;


-- F_amento_capital
function CF_INCRE_CAPITALFormula return Char is
 Lv_monto    varchar2(25);
begin
...
-- F_CLIENTE
function CF_CLIENTEFormula return Char is
...
-- F_1
function CF_PROPIETARIOSFormula return Char is
Gv_NombreCliente       varchar2(130);
Gv_CodigoError         varchar2(80);
begin
...
-- F_DIRECCION
function CF_DEPTOFormula return Char is
Gv_Direccion     varchar2(200);
Gv_codigoError   varchar2(200);
   
BEGIN
     PU_P_OBTIENE_DIRECCION(:codigo_propietario_principal,:codigo_direccion,:cf_correspondencia,  
